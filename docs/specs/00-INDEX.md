...
| O5 | ~~Droplet automation~~ **RESOLVED 2026-07-09 (recon)** — `doctl` API path confirmed viable: `doctl compute droplet create --api-url https://api-amd.digitalocean.com --size gpu-mi300x8-1536gb` (custom `--api-url` mandatory). Fleet manager gets real create/drain/destroy; idle watchdog may auto-destroy. Manual portal remains the fallback | needs `DIGITALOCEAN_ACCESS_TOKEN` | done |
