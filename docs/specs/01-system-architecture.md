...
|---|---|---|---|
| **Control VM** ("platform host") | Always-on address of the product | FastAPI control plane, orchestrator, SQLite, built frontend, Caddy, process-runtime containers, code-exec sandbox containers, local Whisper (policy dictation, O9) | 24/7 — **DigitalOcean s-2vcpu-4gb in `nyc1`** (provisioned 2026-07-09: same account/billing/`doctl` as the MI300X fleet. **Not co-located**: the MI300X Dev Cloud is `atl1`-only and `atl1` sells no standard droplet sizes, so the VM sits in `nyc1` — nearest US-East, ~15–20 ms to atl1, fine for a network-bound control plane. Reserved IP for rebuild-safe DNS; Hetzner CX32 / Fly.io = fallback) |
