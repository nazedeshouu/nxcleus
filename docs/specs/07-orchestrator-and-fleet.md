...
### 2026-07-10 hardening wave — createTool() primitive (T8)

`orchestrator/toolsmith.py`: `create_tool(purpose, args_example, scope, complete_fn, agent_dir)` dispatches one coder-seat completion for a self-contained tool file (`TOOL` + `SELF_TEST` + `run(args)->dict`; stdlib+sqlite3 only, no network, deterministic), SELF-TESTS it in the code-exec sandbox (agent-folder mount only, network none, 20s), one repair round on failure, and registers only passed tools in the new `tools` table. `invoke_tool(scope, name, args)` runs the registered tool in the same sandbox (args via file, stdout JSON) and emits `tool.invoked`. Consumers: the stage-6 inspector loop (create_tool/call_tool actions; commissioned tools join the injected tools dict mid-loop, shared `agents/inspector-0` folder) and topology `kind:"analysis"` candidate steps (tool cached per scope+step id; `{"rows":[...]}` → `{"findings":[...]}` replaces the unit set like sql rows). Surfaces: `GET /api/tools?scope=`, events `tool.created` {name, description, agent} and `tool.invoked` {name, ms, ok}. Ceiling: routing is the plain coder seat (no per-dispatch pool-member override yet); without docker, tools are honestly refused, never trusted untested.
