...
function BuildSection({
  ...
  return (
    <details
      className="bv-phase-group"
      open={open}
      onToggle={(event) => setOpen(event.currentTarget.open)}
    >
      <summary>
        <span className="bv-phase-title">{title}</span>
        <span className="bv-phase-meta">{meta}</span>
        <CaretDown weight="bold" />
      </summary>
      <div className="bv-phase-body">{children}</div>
    </details>
  );
}
...
export function CockpitFrame({ view, top, jobId, completion }: { view: JobView; top: ReactNode; jobId?: string; completion?: ReactNode }) {
  ...
  const settled = view.status === "done" || view.status === "delivered";
  return (
    <div className="bv-root">
      ...
      {jobId && (
        <Link
          to={`/build/${jobId}/map`}
          className="bv-map-link"
        >
          <TreeStructure weight="bold" /> View run map
        </Link>
      )}
      {completion}
      ...
      <div className="bv-workspace">
        ...
        {settled && <DeliveryMoment view={view} />}

        <BuildSection
          title="Request and plan"
          meta="Intent, boundary, plan and certification"
          active={!settled && view.stage <= 2}
        >
        ...
        <BuildSection
          ...
          meta="Modules, tests and adversarial QA"
          active={!settled && view.stage >= 3}
        >
          <WaveBoard view={view} />
        ...
        <BuildSection
          ...
          meta="Amendments, compute and network activity"
          active={false}
        >
          <div className="bv-tech-grid">
            <AuditRail view={view} />
            <Telemetry view={view} />
  ...
  return (
    <div className="bv-complete-bar" role="status">
      ...
      <Link to={`/traces?scope=${encodeURIComponent(`job:${jobId}`)}`} className="bv-complete-btn bv-complete-ghost">
        <MagnifyingGlass weight="bold" /> View model activity
      </Link>
    </div>
  );
}
