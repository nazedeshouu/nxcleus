...
// ponytail: the seeded reasoning/unstructured tasks are long multi-clause instructions (read a
// narrative, trace a multi-hop chain, resolve entities across records); deterministic SQL tasks
// are one-liners. Length cleanly separates the two across every seeded prompt (docs/demo-datasets.md:
// longest simple 166 chars, shortest reasoning 195). Auto-generated BYOD prompts are short → simple,
// the safe default. Upgrade to a backend `reasoning` flag if BYOD ever needs the distinction.
export const isReasoningPrompt = (prompt: string): boolean => prompt.length > 170;
